...
static void _quit(int ret)
{
	...
	if (en_debug) printf("Setting process-wide thread cancelation policy...\n");
	pthread_setcancelstate(PTHREAD_CANCEL_ENABLE, NULL);
	pthread_setcanceltype(PTHREAD_CANCEL_ASYNCHRONOUS, NULL);

	//FORCE KILL THREADS CREATED BY THE PIPE CLIENT
	if (en_debug) printf("Forcibly closing all pipe connections...\n");
	pipe_server_close_all();
	pipe_client_close_all();
	
	//WAIT FOR THREADS TO TERMINATE
	if (en_debug) printf("Waiting for threads to terminate...\n");
	usleep(100000);  // 100ms
	...
	if (en_debug) printf("Forcibly exiting process now!\n");
...
static int connect_client_pipes(void)
{
	...
	if (ret == 0) {
		...
		for (const auto& camera : voxl::CameraManager::getInstance().getAllCameras()) {
			if (i < MAX_CAM_CNT) { //6 CAMERAS HARDCODED MAXIMUM 
...
int main(int argc, char *argv[])
{
	...
	vio_manager = std::unique_ptr < VioManager
			> (new VioManager(vio_manager_options));

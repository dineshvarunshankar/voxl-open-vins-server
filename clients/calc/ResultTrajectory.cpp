...
ResultTrajectory::ResultTrajectory(std::string path_est, std::string path_gt, std::string alignment_method) {
  ...
  if (est_poses.size() < 3) {
    ...
    fprintf(stderr, "[TRAJ]: does the estimated trajectory publish the rosbag timestamps??\n" );
...
void ResultTrajectory::calculate_rpe(const std::vector<double> &segment_lengths,
  ...
  if (average_pos_difference > max_dist_diff) {
    fprintf(stderr, "[COMP]: average groundtruth position difference %.2f > %.2f is too large\n" , average_pos_difference,
                  max_dist_diff);
    fprintf(stderr, "[COMP]: this will prevent the RPE from finding valid trajectory segments!!!\n" );
    fprintf(stderr, 
...
void ResultTrajectory::calculate_nees(Statistics &nees_ori, Statistics &nees_pos) {
  ...
  if (est_times.size() != est_covori.size() || est_times.size() != est_covpos.size() || gt_times.size() != gt_covori.size() ||
    ...
    fprintf(stderr, "[TRAJ]: Normalized Estimation Error Squared called but trajectory does not have any covariances...\n" );
    fprintf(stderr, "[TRAJ]: Did you record using a Odometry or PoseWithCovarianceStamped????\n" );
    return;
  }
  ...
  for (size_t i = 0; i < est_poses.size(); i++) {
    ...
    if (std::isnan(ori_nees) || std::isnan(pos_nees)) {
      fprintf(stderr, "[TRAJ]: nees calculation had nan number (covariance is wrong?) skipping...\n" );
      continue;
    }
